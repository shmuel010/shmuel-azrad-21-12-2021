...
export const getCurrentWeather = (city) => async (dispatch) => {
	...
	try {
		...
		let res = await axiosWeatherCall(`/currentconditions/v1/${newKey}`);
		if (res.status === 200 && res.data.length) {
			...
			res = await axiosWeatherCall(`/forecasts/v1/daily/5day/${newCurrentWeather.key}`, { params: { metric: true } });
...
export const updateDailyWeather = (key) => async (dispatch) => {
	try {
		dispatch({ type: START_LOADING });
		const { favoriteWeathers } = store.getState().weather;
		const res = await axiosWeatherCall(`/currentconditions/v1/${key}`);
		if (res.status === 200 && res.data.length) {
...
export const getUserWeatherLocation = (position) => async (dispatch) => {
	try {
		...
		const res = await axiosWeatherCall('/locations/v1/cities/geoposition/search', {
			params: {
